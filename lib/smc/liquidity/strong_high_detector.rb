...
module SMC
  module Liquidity
    class StrongHighDetector
      attr_reader :displacement, :max_displacement_bars, :max_bos_bars,
                  :accept_choch_as_structure_break, :strong_highs, :protected_high
      ...
      def evaluate_swing(candles, swing, bos_events:, choch_events: [])
        return nil unless swing.type == :high
        ...
        build_strong_high(swing, displacement_event, structure_break)
      end

      def detect_all(candles, swings, bos_events:, choch_events: [])
        Array(swings).compact
          .select { |swing| swing.type == :high }
          .filter_map do |swing|
      ...
      def find_structure_break_after(bos_events:, choch_events:, after_index:)
        bearish_bos = bos_events.select do |event|
        ...
        if bearish_bos.any?
          event = bearish_bos.min_by { |candidate| candidate[:index] }
          return { event: event, type: :bos }
        end

        return nil unless @accept_choch_as_structure_break

        bearish_choch = choch_events.select do |event|
          event[:direction] == :bearish &&
            event[:index] > after_index &&
            event[:index] <= after_index + @max_bos_bars
        end
        return nil if bearish_choch.empty?

        event = bearish_choch.min_by { |candidate| candidate[:index] }
        { event: event, type: :choch }
      end

      def build_strong_high(swing, displacement_event, structure_break)
        {
          swing: swing,
          price: swing.price,
